...
// Update the edge function for video analysis
<lov-write file_path="supabase/functions/analyze-video-evidence/index.ts">
...
async function tryLocalModel(videoUrl: string, location?: string): Promise<any | null> {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Analyzing video: ${videoUrl} for report: ${reportId || 'N/A'}, location: ${location || 'N/A'}`);
    ...
    let modelResult = await tryLocalModel(videoUrl, location);
    ...
    if (reportId) {
      try {
        ...
        // Update report with location if provided
        if (location) {
          await supabase
            .from('crime_reports')
            .update({ 
